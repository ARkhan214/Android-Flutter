...
class InternetBillPage extends StatefulWidget {
  const InternetBillPage({super.key});
...
class _InternetBillPageState extends State<InternetBillPage> {
  ...
  // Internet Service Provider তালিকা
  ...
  Future<void> _loadInitialData() async {
    ...
    final savedForm = prefs.getString('internetBillForm');
    if (savedForm != null) {
  ...
  void _saveForm() async {
    ...
    prefs.setString('internetBillForm', jsonEncode(formData));
  }
  ...
  void _onSubmit() async {
    ...
    final transaction = Transaction(
      id: 0,
      type: 'INTERNET', // Angular-এর type: 'INTERNET' এর সমতুল্য
      amount: amount,
      companyName: _selectedCompany,
      accountHolderBillingId: _billingIdController.text,
    ...
    try {
      // API কল: payInternet ব্যবহার করা হয়েছে
      final res = await _billPaymentService.payInternet(transaction.toJson(), _token);

      _showSnackbar('${res.amount.toStringAsFixed(2)} Taka Internet Bill Payment successful!', isError: false);
      _resetForm();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('🌐 Internet Bill Payment'),
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildLabel('Internet Service Provider', primaryColor),
